...
function buildWorkOrder(call, extracted, bossId) {
  ...
  return {
    ...
    // Also extract the rich summary (name, tendency, topics, follow-up) from
    // the transcript using call-summary.js (the JS port of src/lib/call-summary.ts)
...
/**
 * Run call-summary logic on the raw transcript (Vapi's `artifact.transcript`
 * is a flat string with "AI:" / "User:" prefixes) and return the additional
 * work_order fields it generates (customer_name_extracted, intent_summary,
 * customer_tendency, mentioned_topics, follow_up_*).
 */
function summarizeCallToWorkOrderFields(extracted, call) {
  // Convert raw transcript string to [{role, text}] format for call-summary
  let transcript = null;
  if (extracted.transcript && typeof extracted.transcript === "string") {
    ...
    // Vapi sometimes returns messages array directly
  ...
  // Use the same LLM-with-fallback path as the live webhook.
